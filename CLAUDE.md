...
- Any screen that lives in a subfolder (e.g. `sistema/settings/colores.php`) needs `<base href="/">` in `includes/header.php` to keep the app's bare-relative asset/link paths (`assets/css/style.css`, `menu_opciones.php`, `logout.php`, etc.) resolving against the `sistema/` root instead of the subfolder — already in place; don't remove it. It also means `sistema/` must actually be served at the site root for those pages to load correctly (per the deployment model in "Running the app" above).
